...
def run_benchmark(mode_name, simultaneous, async_training, epochs=300):
    ...
    # Construct command
    cmd = [
        "poetry", "run", "python", "sorrel/examples/iowa/main.py",
    ...
    while True:
        ...
        if line:
            print(line.strip())
            # Parse reward
            # Format: Epoch 10: Avg Reward (last 11): -52.22, Avg Loss: 0.11, Epsilon: 0.5000
...
def main():
    ...
    # Save results
    results_dir = Path("benchmark_results_iowa")
    ...
    plt.title("Iowa Learning Curve Comparison (4 Versions)")
    plt.legend()
    plt.grid(True)
    plt.savefig(results_dir / "iowa_rewards_comparison_4v.png")
