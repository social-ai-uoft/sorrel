...
class Agent:
        ...
        if str(target_object) in state_sys.potential_taboo:
            ...
            if mode == 'certain':
                ...
                if state_sys.resource_punishment_schedule_is_dynamic:
                    ...
                    r_of_puppet_training = state_sys.magnitude * (random.random() < punishment_prob)
                    reward -= r_of_puppet_training
                else:
                    if state_sys.only_punish_taboo:
                        if str(target_object) in state_sys.taboo:
                            ...
                            r_of_puppet_training = 0
                    else:
                        r_of_puppet_training = state_sys.magnitude * (random.random() 
                                                        < state_sys.prob_list[str(target_object)]*state_sys.prob) # instant punishment
                        reward -= r_of_puppet_training
                # record being punished or not
                self.punishment_record.append(1*(r_of_puppet_training > 0))
    ...
    #     # ll
    #     # puppet_training_prob_tensor = torch.full((state.shape()[1], state.shape()[2]), state_sys.prob).view(1, -1)
    #     # model_input = torch.concat([model_input, puppet_training_prob_tensor])
    #     reward = 0
