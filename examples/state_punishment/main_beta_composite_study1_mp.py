...
# ------------------------
# GLOBAL model registry (one per worker process)
# ------------------------
GLOBAL_MODELS = {}

def init_worker(model_dict):
    """Initializer function to set up global model state in each worker."""
    global GLOBAL_MODELS
    GLOBAL_MODELS = model_dict


def process_agent(agent_ixs):
    """Worker function to train a model and return detached loss."""
    global GLOBAL_MODELS
    model = GLOBAL_MODELS[agent_ixs]

    # show memory length
    # print(f"Memory length: {model.memory.idx}")

    loss = model.train_model()
    return (agent_ixs, loss.detach())

# ------------------------

# def process_agent(agent):
    
#     # Run training and detach loss
#     loss = agent.model.train_model()
#     loss = loss.detach()

#     return (agent.ixs, loss)

# def mp_train(agent_list, losses):

#     with mp.Pool(processes=mp.cpu_count()) as pool:
#         results = pool.map(process_agent, agent_list)

#     # Collect results and update losses + record
#     for ixs, loss in results:
#         losses[ixs] += loss
    
#     return losses

# def mt_train(agent_list, losses):
#     with concurrent.futures.ThreadPoolExecutor(max_workers=mp.cpu_count()) as executor:
#         results = list(executor.map(process_agent, agent_list))
...
def run(cfg, **kwargs):
    ...
    if cfg.load_weights:
        ...
        for count, agent in enumerate(agents):
            agent.model.load(
                f'{root}/examples/state_punishment/models/checkpoints/test_study1__fixed_only_taboo_stacked_view_simple_actions_3agents_respawn_0.04_v0_s3_r2_seed3_agent{agent.ixs}_iRainbowModel.pkl')
    else:
        assert 's1' in cfg.exp_name, ValueError('incorrect configurations')
    ...
    model_dict = {agent.ixs: agent.model for agent in agents}

    ctx = mp.get_context("spawn")  # safer context for ML
    with ctx.Pool(initializer=init_worker, initargs=(model_dict,), processes=mp.cpu_count()) as pool:
        ...
        for epoch in range(cfg.experiment.epochs):
            ...
            ## mp training
            # losses = mp_train(agents, losses)

            ## mt training
            # losses = mt_train(agents, losses)

            ## v2: mp training
            global GLOBAL_MODELS
            GLOBAL_MODELS = {agent.ixs: agent.model for agent in agents}

            # Prepare per-epoch agent input
            agent_input_data = [
                (agent.ixs,)
                for agent in agents
            ]
            ...
            results = pool.starmap(process_agent, agent_input_data)
