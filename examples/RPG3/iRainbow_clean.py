...
class IQN(nn.Module):
    ...
    def __init__(
        ...
        self.action_size = cfg.model.agent.parameters.action_size
        self.n_quantiles = cfg.model.agent.parameters.n
        self.n_cos = 64
        self.layer_size = cfg.model.agent.parameters.layer_size
        self.pis = (
            torch.FloatTensor([np.pi * i for i in range(1, self.n_cos + 1)])
            .view(1, 1, self.n_cos)
            .to(device)
        )
        self.use_cnn = cfg.model.agent.parameters.use_cnn
        self.device = device

        # Network architecture
        if self.use_cnn:
            self.cnn = CNN_CLD(cfg.model.agent.parameters.in_channels, cfg.model.agent.parameters.num_filters)
            self.head1 = nn.Linear(cfg.model.agent.parameters.cnn_out_size, cfg.model.agent.parameters.layer_size)
        else:
            self.head1 = nn.Linear(1134, cfg.model.agent.parameters.layer_size)

        self.cos_embedding = nn.Linear(self.n_cos, cfg.model.agent.parameters.layer_size)
        self.ff_1 = NoisyLinear(cfg.model.agent.parameters.layer_size, cfg.model.agent.parameters.layer_size)
        self.cos_layer_out = cfg.model.agent.parameters.layer_size

        self.advantage = NoisyLinear(cfg.model.agent.parameters.layer_size, cfg.model.agent.parameters.action_size)
        self.value = NoisyLinear(cfg.model.agent.parameters.layer_size, 1)
...
class ReplayBuffer:
    ...
    def __init__(self, cfg, device, seed):
        ...
        """
        self.device = device
        self.memory = deque(maxlen=cfg.model.agent.parameters.replay_size)
...
class iRainbowModel:
    ...
    def __init__(
        ...
        ======
            ...
            seed (int): random seed
        """
        self.state_size = torch.tensor(make_tuple(cfg.model.agent.parameters.state_size))
        self.action_size = cfg.model.agent.parameters.action_size

        self.seed = random.seed(seed)
        self.device = device
        self.TAU = cfg.model.agent.parameters.tau
        self.N = cfg.model.agent.parameters.n
        self.GAMMA = cfg.model.agent.parameters.gamma
        self.BATCH_SIZE = cfg.model.agent.parameters.batch_size
        self.n_step = cfg.model.agent.parameters.n_step
        ...
        self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=cfg.model.agent.parameters.lr)
