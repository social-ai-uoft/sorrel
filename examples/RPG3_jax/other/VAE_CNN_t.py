...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        pe = self.pe[
            :seq_len, :d_model
        ]  # Slice the positional encoding to match input size
        x = x + pe.unsqueeze(1)  # Add positional encoding
        return x
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # Positional Encoding
        self.pos_encoder1 = PositionalEncoding(64)
        ...
        # Final layers
        self.fc_mu = nn.Linear(128 * 2 * 2, 256)
        self.fc_logvar = nn.Linear(128 * 2 * 2, 256)
    ...
    def forward(self, x):
        ...
        x2_flat = x2.flatten(2).permute(2, 0, 1)
        x2_flat = self.pos_encoder1(x2_flat)

        # First Transformer Encoder Layer
        x3 = self.transformer_encoder1(x2_flat)

        # Reshape back for CNN and add skip connection
        x3_reshaped = x3.permute(1, 2, 0).reshape(x.size(0), 64, 4, 4)
        x3_skip = x3_reshaped + x2  # Skip connection

        # Second CNN layer after Transformer
        x4 = F.relu(self.bn3(self.conv3(x3_skip)))

        # Reshape and add positional encoding for second Transformer
        x4_flat = x4.flatten(2).permute(2, 0, 1)
        x4_flat = self.pos_encoder2(x4_flat)

        # Second Transformer Encoder Layer
        x5 = self.transformer_encoder2(x4_flat)

        # Flatten and Final layers
        x5_reshaped = x5.permute(1, 0, 2).reshape(x.size(0), -1)

        mu = self.fc_mu(x5_reshaped)
        logvar = self.fc_logvar(x5_reshaped)
        return mu, logvar
...
def evaluate_model(letter_predictor, vae, letter_probs, num_samples=10000):
    ...
    masked_aliens = apply_mask(aliens)
...
for repeats in range(3):
    train_vae(vae, dataloader, num_epochs=200, eval_sync=100)  # Training
