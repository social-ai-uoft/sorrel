...
class MultiAgentEnv:
    ...
    def relative_team_points(self, team_index):
        # Normalize the scores
        team_points = self.team_points
        min_val = min(team_points)
        max_val = max(team_points)
        if max_val == min_val:
            normalized_scores = [1 if x == max_val else 0 for x in team_points]
        else:
            normalized_scores = [
                (x - min_val) / (max_val - min_val) for x in team_points
            ]
        ...
        # Reorder the list
        team_score = normalized_scores.pop(team_index)
        normalized_scores.sort(reverse=True)
        reordered_scores = (
            [team_score] + normalized_scores + [log_diff / 10]
...
for epoch in range(1000000000):
    ...
    for round in range(100):
        ...
        for pair in pairings:
            ...
            adv_agent_state = env.agents[restricting_agent][
                "available_actions"
            ] + env.relative_team_points(env.agents[restricting_agent]["team"])

            adv_agent_state = (
                torch.tensor(adv_agent_state)
                .float()
                .reshape(
                    8,
                )
            ...
            action_mask = np.array([1, 1, 1])
            action_mask[restricted_action] = 0

            env.agents[restricted_agent]["available_actions"][restricted_action] = 0

            disadv_agent_state = env.agents[restricted_agent][
                "available_actions"
            ] + env.relative_team_points(env.agents[restricted_agent]["team"])

            disadv_agent_state = (
                torch.tensor(disadv_agent_state)
                .float()
                .reshape(
                    8,
                )
        ...
        revolution, redistribution = env.check_revolution(
            revolution_prercentage=0.25
        )  # temporary setting so no revolutions
        if revolution == 1:
            revolutions += 1
            for agent in range(env.num_agents):
                if len(env.agents[agent]["replay_buffer"]) > 0:
                    env.agents[agent]["replay_buffer"][-1][2] = redistribution
        if revolution == 0:
            for agent in range(env.num_agents):
                if len(env.agents[agent]["replay_buffer"]) > 0:
                    if env.agents[agent]["replay_buffer"][-1][1] == 2:
                        env.agents[agent]["replay_buffer"][-1][
                            2
                        ] = -10  # revolutionaries get punished

        for agent in adv_players:
            net.push_to_buffer(env.agents[agent]["replay_buffer"][-1], restricted=False)
