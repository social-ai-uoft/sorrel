...
class DynamicModel:
    ...
    def train_failing(self, restricted, batch_size=32, gamma=0.0):
        ...
        # Double DQN: Use target network for next state Q-values
        if restricted:
            next_q_values = self.forward(next_states, restricted, target=True)
        else:
            next_q_values, _ = self.forward(next_states, restricted, target=True)

        actions = actions.unsqueeze(1).unsqueeze(2)
        next_q_values = next_q_values.gather(2, actions).squeeze(2).squeeze(1)

        # next_q_values = next_q_values.gather(1, actions.unsqueeze(1)).squeeze(1)
        if len(next_q_values.shape) == 1:
            max_next_q_values, _ = next_q_values.max(0)
        elif len(next_q_values.shape) == 2:
        ...
        if restricted:
            predicted_q_values = self.forward(states, restricted)
        else:
            # this doesn't seem right. It is not usin the restriction
            # makes me wonder if a 9 output head and a 3 output head would be
            # more appropriate
            predicted_q_values, _ = self.forward(states, restricted)

        predicted_q_value = predicted_q_values.gather(2, actions).squeeze(2).squeeze(1)

        # predicted_q_value = predicted_q_values.gather(1, actions.unsqueeze(1)).squeeze(1)

        # Compute loss
        loss = self.criterion(predicted_q_value, target_q_values.detach())
