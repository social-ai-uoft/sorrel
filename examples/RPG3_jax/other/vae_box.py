...
def apply_mask_blur(images, mask_size=3, mask_percentage=0.5):
...
def apply_mask(images, mask_size=2, mask_percentage=0.5, x=None, y=None):
    masked_images = images.clone()
    # Mask everything in black
    masked_images.fill_(0.0)

    for i in range(images.size(0)):
        # If x and y are not provided, generate random coordinates
        if x is None or y is None:
            h = torch.randint(0, images.size(2), (1,)).item()
            w = torch.randint(0, images.size(3), (1,)).item()
        else:
            h, w = y, x

        # Calculate the boundaries of the box
        h_start = max(h - mask_size, 0)
        h_end = min(h + mask_size + 1, images.size(2))
        w_start = max(w - mask_size, 0)
        w_end = min(w + mask_size + 1, images.size(3))

        # Apply the box mask
        masked_images[i, :, h_start:h_end, w_start:w_end] = images[
            i, :, h_start:h_end, w_start:w_end
        ]
