...
# Loss Function
def loss_function(recon_x, x, mu, logvar, class_output, class_target):
    BCE = F.binary_cross_entropy(recon_x, x, reduction="sum")
    KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    classification_loss = F.cross_entropy(class_output, class_target)
    return BCE + KLD + 0.25 * classification_loss
...
def train_vae(vae, dataloader, num_epochs, eval_sync=100):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % eval_sync == 0:
            ...
            eval_dat = evaluate_model(vae, letter_probs, num_samples=10000)
            print(eval_dat)
...
def visualize_reconstruction(vae, images, num_samples=5):
    with torch.no_grad():  # Disable gradient computation
        ...
        reconstructed_images, _, _, letter_guess = vae(masked_images)
...
def evaluate_model(vae, letter_probs, num_samples=10000):
    # Generate aliens
    ...
    with torch.no_grad():
        reconstructed_images, _, _, letter_guess = vae(masked_aliens)
...
visualize_reconstruction(vae, evaluation_images, num_samples=30)
