...
class Encoder(nn.Module):
    def __init__(self):
        ...
        # Patch extraction
        self.patch_dim = 8  # Size of each patch (16x16)
...
class MultiPatchEncoder(nn.Module):
    def __init__(self):
        super(MultiPatchEncoder, self).__init__()

        # Initialize patch dimensions and transformer encoders for 4x4, 8x8, and 16x16 patches
        self.patch_dims = [4, 8, 16]
        self.transformer_encoders = nn.ModuleDict(
            {
                f"{dim}": TransformerEncoder(
                    TransformerEncoderLayer(d_model=dim * dim * 3, nhead=4),
                    num_layers=3,
                )
        ...
        self.fc_mu = nn.Linear(
            2304, 256
        )  # Update the input dimension based on your specific architecture
        self.fc_logvar = nn.Linear(2304, 256)

    def extract_patches(self, x, patch_dim):
        batch_size, C, H, W = x.size()
        num_patches = (H // patch_dim) * (W // patch_dim)
        flatten_dim = patch_dim * patch_dim * C

        x = x.unfold(2, patch_dim, patch_dim).unfold(3, patch_dim, patch_dim)
        x = x.permute(0, 2, 3, 1, 4, 5).reshape(batch_size, num_patches, flatten_dim)

        return x

    def forward(self, x):
        # Initialize a list to hold the flattened outputs from each transformer encoder
        flattened_outputs = []

        # Loop through each patch size
        for dim in self.patch_dims:
            # Extract patches and pass through corresponding transformer
            x_patch = self.extract_patches(x, dim)
            x_transformed = self.transformer_encoders[f"{dim}"](x_patch)

            # Flatten the output and add to the list
            x_flattened = x_transformed.reshape(x_transformed.size(0), -1)
            flattened_outputs.append(x_flattened)

        # Concatenate the flattened outputs
        x_concat = torch.cat(flattened_outputs, dim=1)

        # Fully connected layers
        mu = self.fc_mu(x_concat)
        logvar = self.fc_logvar(x_concat)
...
class VAE(nn.Module):
    def __init__(self, num_classes=4):  # Assuming 4 classes for letters A, b, d, E
        ...
        self.encoder = MultiPatchEncoder()
        # self.decoder = TransformerDecoder()  # Use the new TransformerDecoder
        self.decoder = Decoder()  # Use the new TransformerDecoder
        self.middle = Middle()
...
def apply_mask(images, mask_size=3, mask_percentage=0.5):
    ...
    for i in range(images.size(0)):
        ...
        if torch.rand(1).item() < mask_percentage:
            ...
            # masked_images = masked_images.to('cpu')
...
def train_vae(vae, dataloader, num_epochs, eval_sync=100):
    ...
    for epoch in range(num_epochs):
        for batch in dataloader:
            masked_data, unmasked_data, class_target = batch
            # Move data to GPU
            masked_data, unmasked_data, class_target = (
                masked_data.to(device),
                unmasked_data.to(device),
                class_target.to(device),
            )
            optimizer.zero_grad()
        ...
        if epoch % eval_sync == 0:
            evaluation_images = generate_random_aliens(15, letter_probs).to(device)
            ...
            eval_dat = evaluate_model(
                letter_predictor, vae, letter_probs, num_samples=1000
            )
            print(eval_dat)
...
def visualize_reconstruction(
    letter_predictor, vae, images, num_samples=5, device=device
):
    with torch.no_grad():
        images = generate_random_aliens(15, letter_probs)
        masked_images = apply_mask(images)
        reconstructed_images, _, _, _ = vae(masked_images.to(device))
        ...
        probabilities = torch.softmax(letter_guess, dim=1).detach().cpu().numpy()
    ...
    for i in range(num_samples):
        axs[i, 0].imshow(images[i].permute(1, 2, 0).cpu() * rescale_factor)
        axs[i, 0].axis("off")
        axs[i, 0].set_title("Original Image")

        axs[i, 1].imshow(masked_images[i].permute(1, 2, 0).cpu() * rescale_factor)
        axs[i, 1].axis("off")
        axs[i, 1].set_title("Masked Image")
        axs[i, 2].imshow(
            reconstructed_images[i].permute(1, 2, 0).cpu() * rescale_factor
        )
        axs[i, 2].axis("off")
...
def evaluate_model(
    letter_predictor, vae, letter_probs, num_samples=1000, mask_percentage=0.5
):
    # Generate aliens
    ...
    with torch.no_grad():
        reconstructed_images, _, _, _ = vae(masked_aliens.to(device))
        top5_lines = reconstructed_images[:, :, :5, :]
...
class LetterPredictor(nn.Module):
    def __init__(self, device):
        ...
        self.fc1 = nn.Linear(16 * 5 * 16, 128).to(device)
        self.fc2 = nn.Linear(128, 4).to(
            device
        )  # Assuming 4 classes for letters A, b, d, E
    ...
    def train_model(self, vae, dataloader, num_epochs=10):
        ...
        for epoch in range(num_epochs):
            for batch in dataloader:
                ...
                masked_data, class_target = masked_data.to(
                    self.device
                ), class_target.to(self.device)
...
vae = VAE().to(device)
letter_predictor = LetterPredictor(device)
...
for repeats in range(10):
    train_vae(vae, dataloader, num_epochs=3000, eval_sync=100)  # Training
...
if confounded == True:
    ...
    train_vae(vae, dataloader, num_epochs=5000, eval_sync=100)  # Training
