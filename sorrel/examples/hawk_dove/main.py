...
if __name__ == "__main__":
    ...
    config = {
        "experiment": {
            ...
            "device": "cpu",
            "run_name": f"hawk_dove_{STATIC_RUNTIME}",
        },
        "model": {
            "agent_vision_radius": 2,
            "epsilon_decay": 0.0005,
            "emotion_length": 6,
            "emotion_condition": "full",
        },
        "world": {
            "height": 11,
            "width": 11,
    ...
    # construct the world
    world = HawkDoveWorld(config=config, default_entity=EmptyEntity())
    # construct the environment
    env = HawkDoveEnv(world, config)

    # Initialize metrics collection
    metrics_collector = HawkDoveMetricsCollector()
    env.metrics_collector = metrics_collector

    # run the experiment
    env.run_experiment(
        logger=HawkDoveLogger(
