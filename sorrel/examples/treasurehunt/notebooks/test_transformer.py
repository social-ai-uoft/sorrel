...
if __name__ == "__main__":
    ...
    if not SKIP_IQN:
        ...
        model_path = (
            DATA_DIR / "checkpoints" / f"treasurehunt_model_{STATIC_RUNTIME}.pkl"
        )
    ...
    for mask_type in EVAL_MASK_TYPES:
        ...
        logger.writer.add_scalar(
            f"eval_summary/{mask_type}_avg_state_loss", avg_state, 0
        )
        logger.writer.add_scalar(
            f"eval_summary/{mask_type}_avg_action_loss", avg_action, 0
        )
        logger.writer.add_scalar(
            f"eval_summary/{mask_type}_avg_total_loss", avg_total, 0
        )
        logger.writer.add_scalar(
            f"eval_summary/{mask_type}_std_total_loss", std_total, 0
        )
        ...
        for ch, loss_val in per_channel.items():
            ...
            logger.writer.add_scalar(
                f"eval_{mask_type}/per_channel/{name}", loss_val, 0
            )
