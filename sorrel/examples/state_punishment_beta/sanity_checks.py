...
and edge cases to ensure the state punishment implementation works correctly.
"""
...
from sorrel.examples.state_punishment_beta.entities import A, B, C, D, E, EmptyEntity, Wall
...
def test_entity_creation():
    ...
    for i, (entity, exp_val, exp_harm, exp_kind) in enumerate(zip(entities, expected_values, expected_harm, expected_kinds)):
        results.add_result(
            f"Entity {exp_kind} creation",
            entity.value == exp_val and entity.social_harm == exp_harm and entity.kind == exp_kind,
...
def test_state_system_initialization():
    ...
    results.add_result(
        ...
        state_custom.prob == 0.5 and state_custom.magnitude == -5.0 and 
        state_custom.change_per_vote == 0.1 and state_custom.taboo_resources == ["A", "B"],
...
def test_social_harm_system():
    ...
    results.add_result(
        "Social harm from entity A",
        harm_a == 0.5,
        f"Expected: 0.5, Got: {harm_a}"
    )
    
    results.add_result(
        "Social harm from entity B",
        harm_b == 1.0,
        f"Expected: 1.0, Got: {harm_b}"
    )
    
    results.add_result(
        "Social harm from entity D",
        harm_d == 1.5,
        f"Expected: 1.5, Got: {harm_d}"
    )
...
def test_resource_spawning():
    ...
    config = get_test_config(
        experiment={"num_agents": 1, "max_turns": 100},
        world={"height": 5, "width": 5, "spawn_prob": 1.0}  # 100% spawn probability for testing
    )
...
def test_edge_cases():
    ...
    try:
        ...
        results.add_result(
            ...
            True,
            "Single turn completed successfully"
        )
...
def test_learning_mechanics():
    ...
    config = get_test_config(
        experiment={"num_agents": 1, "max_turns": 10},
        world={"height": 5, "width": 5}
    )
    ...
    try:
        ...
        for epoch in range(5):
            env.take_turn()
            if hasattr(agent.model, 'loss_history') and len(agent.model.loss_history) > 0:
        ...
        if hasattr(agent.model, 'memory') and len(agent.model.memory) > 0:
            results.add_result(
                "Agent memory usage",
                True,
                f"Memory size: {len(agent.model.memory)}"
            )
        else:
            results.add_warning(
                "Agent memory usage",
                "Agent memory appears to be empty"
            )
    ...
    for test_func in test_functions:
        ...
        try:
            ...
            all_results.add_result(
                test_func.__name__,
                False,
                f"Test function failed with error: {str(e)}"
            )
