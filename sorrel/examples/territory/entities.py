...
class Province(Entity[TerritoryWorld]):
    ...
    def harvest(self) -> int:
        ...
        self.invade_counter = np.random.randint(
            2, self.invade_cooldown
        )  # max(3, self.invade_counter)
        return reward
    ...
    def plan_attack(self, world: TerritoryWorld) -> list[Vector | Location] | None:
        ...
        target_entities = [
            (
                world.observe(forward)
                if (
                    world.valid_location(forward)
                    and (not any(number < 0 for number in forward))
                )
                else self
            ),
            (
                world.observe(backwards)
                if (
                    world.valid_location(backwards)
                    and (not any(number < 0 for number in backwards))
                )
                else self
            ),
            (
                world.observe(left)
                if (
                    world.valid_location(left)
                    and (not any(number < 0 for number in left))
                )
                else self
            ),
            (
                world.observe(right)
                if (
                    world.valid_location(right)
                    and (not any(number < 0 for number in right))
                )
                else self
            ),
        ]
        for i in range(len(target_entities)):
            ...
            while isinstance(target_entities[i], River) and in_bounds:
                ...
                if i == 0:
                    ...
                    y += 1
                in_bounds = world.valid_location((x, y, z)) and (
                    not any(number < 0 for number in (x, y, z))
                )
        ...
        target_entities = [
            e for e in target_entities if isinstance(e, Province) and e is not self
        ]
        target_entities = [e for e in target_entities if e.side != self.side]
        ...
        if self.invade_counter == 0 and len(target_entities) > 0:
            ...
            d = distance(
                Location(target_province.location[0], target_province.location[1]),
