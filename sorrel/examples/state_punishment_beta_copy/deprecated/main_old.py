...
from sorrel.examples.state_punishment_beta_copy.env import (
...
from sorrel.examples.state_punishment_beta_copy.logger import StatePunishmentLogger as ModularLogger
...
def main(
    ...
    # Create shared state system and social harm dictionary (once, outside the loop)
    temp_world = StatePunishmentWorld(config=config, default_entity=EmptyEntity())
    shared_state_system = temp_world.state_system
    shared_social_harm = {j: 0.0 for j in range(num_agents)}
    ...
    for i in range(num_agents):
        ...
        # Note: Individual worlds don't need to hold state_system or social_harm
        # They will be managed centrally by MultiAgentStatePunishmentEnv
...
def main_simplified():
    ...
    parser = argparse.ArgumentParser(description="Run state punishment experiments (simplified)")
    ...
    # Create configuration using modular function
    config = create_config(
        num_agents=args.num_agents,
        epochs=args.epochs,
        simple_foraging=args.simple_foraging,
    ...
    # Set up environments using modular function
    multi_agent_env, shared_state_system, shared_social_harm = setup_environments(
        config, args.simple_foraging, args.fixed_punishment, False
    )
    
    # Create logger using modular class
    log_dir = Path("runs")
    logger = ModularLogger(
        max_epochs=args.epochs,
        log_dir=log_dir,
        experiment_name=f"simplified_{args.num_agents}agents"
    )
    logger.set_multi_agent_env(multi_agent_env)
    
    # Run experiment
    print(f"Starting simplified experiment with {args.num_agents} agents")
...
if __name__ == "__main__":
    # Uncomment the line below to use the simplified version
    # main_simplified()
    
    # Original main function (default)
    main()
