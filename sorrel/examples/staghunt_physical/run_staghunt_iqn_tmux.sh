#!/bin/sh
# Detached tmux session: staghunt_physical main.py (PyTorchIQN; run-name-base uses size16 + seed).
#
...
#   STAGHUNT_CONDA_ENV=myenv ./run_staghunt_iqn_tmux.sh
#
# Examples:
#   RESOURCE_DENSITY=0.15 ./run_staghunt_iqn_tmux.sh
#   RESPAWN_RATE=0.02 ./run_staghunt_iqn_tmux.sh
#   ./run_staghunt_iqn_tmux.sh --density 0.15 my_session
#   ./run_staghunt_iqn_tmux.sh --respawn-rate 0.02 my_session
#   NUM_GROUPS=1 NO_PUNISHMENT=0 ./run_staghunt_iqn_tmux.sh
#
...
RUN_NAME_BASE="${NUM_GROUPS}group_iqn_${PUNISH_TAG}_size16_density${DENSITY_TAG}_rr${RESPAWN_TAG}_seed${SEED}"

if [ -n "$SESSION_NAME" ]; then
	:
else
	SESSION_NAME="sh_iqn_${NUM_GROUPS}g_${PUNISH_TAG}_d${DENSITY_TAG}_rr${RESPAWN_TAG}_s${SEED}"
fi
