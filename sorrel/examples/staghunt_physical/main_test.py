...
def run_stag_hunt(
    ...
    observe_global_resource_counts: bool | None = None,
    global_resource_counts_sham: bool | None = None,
    global_resource_counts_include_total: bool | None = None,
    ...
    Args:
        ...
        observe_global_resource_counts: If True, append global resource count scalars to observation; if False, disable; if None, use config default.
        global_resource_counts_sham: If True (and observe_global_resource_counts), append zeros instead of real counts; if False, use real; if None, use config default.
        global_resource_counts_include_total: If True, include total count scalar in addition to stag/hare; if False, only stag/hare; if None, use config default.
        global_resource_count_norm: Optional fallback denominator when world caps are None; if None, use config default.
    ...
    config = {
        ...
        "model": {
            ...
            "GAMMA": 0.99,
            "n_quantiles": 12,
            # PPO only: concat this many single-step observations (1 = disabled). Also settable under model.ppo_cpc.
            "ppo_n_frames": 1,
        ...
        "world": {
            ...
            # Per-tile previous executed action slot (one-hot with A+1 classes; NA for non-agent tiles)
            "observe_prev_actions_tile_enabled": False,
            "observe_prev_actions_tile_obs_enabled": False,
            "observe_prev_actions_tile_obs_randomize": False,
            "observe_prev_actions_tile_obs_random_seed": 0,
            # Global resource count observation (optional; appended as extra scalars after local tail).
            # Use global_resource_counts_sham=True as a placebo to match input width without conveying signal.
            "observe_global_resource_counts": False,
            "global_resource_counts_sham": False,
            "global_resource_counts_include_total": False,  # If True, also include total resource count scalar
            "global_resource_count_norm": None,  # Fallback denominator when caps are None (None => use raw counts)
            # respawn timing
    ...
    # Global resource count observation overrides
    if observe_global_resource_counts is not None:
        config["world"]["observe_global_resource_counts"] = observe_global_resource_counts
    if global_resource_counts_sham is not None:
        config["world"]["global_resource_counts_sham"] = global_resource_counts_sham
    if global_resource_counts_include_total is not None:
        config["world"]["global_resource_counts_include_total"] = global_resource_counts_include_total
...
if __name__ == "__main__":
    ...
    # Global resource count observation (real vs sham control)
    parser.add_argument(
        "--observe-global-resource-counts",
        action="store_true",
        default=False,
        help="Append global resource count scalars to observation (treatment / or sham if enabled).",
    )
    parser.add_argument(
        "--no-observe-global-resource-counts",
        action="store_true",
        default=False,
        help="Disable global resource count scalars in observation (default when neither flag is passed).",
    )
    parser.add_argument(
        "--global-resource-counts-sham",
        action="store_true",
        default=False,
        help="Placebo control: keep global-count scalar channels but force them to 0. Requires --observe-global-resource-counts.",
    )
    parser.add_argument(
        "--no-global-resource-counts-sham",
        action="store_true",
        default=False,
        help="Use real normalized global counts (default when neither sham flag is passed).",
    )
    parser.add_argument(
        "--global-resource-counts-include-total",
        action="store_true",
        default=False,
        help="Include total resource count scalar in addition to stag/hare (adds 1 more scalar).",
    )
    parser.add_argument(
        "--global-resource-count-norm",
        type=float,
        default=None,
    ...
    observe_global_resource_counts = None
    if args.observe_global_resource_counts:
        observe_global_resource_counts = True
    elif args.no_observe_global_resource_counts:
        observe_global_resource_counts = False

    global_resource_counts_sham = None
    if args.global_resource_counts_sham:
        global_resource_counts_sham = True
    elif args.no_global_resource_counts_sham:
        global_resource_counts_sham = False
    ...
    run_stag_hunt(
        ...
        observe_global_resource_counts=observe_global_resource_counts,
        global_resource_counts_sham=global_resource_counts_sham,
        global_resource_counts_include_total=global_resource_counts_include_total,
        global_resource_count_norm=args.global_resource_count_norm,
