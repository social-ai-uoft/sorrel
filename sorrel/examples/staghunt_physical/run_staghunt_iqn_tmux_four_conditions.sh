...
# x (each seed in SEED_LIST). run_staghunt_iqn_tmux.sh sets run-name-base (size16 + seed in name).
#
# Usage:
#   ./run_staghunt_iqn_tmux_four_conditions.sh --density 0.06
#   ./run_staghunt_iqn_tmux_four_conditions.sh --density 0.03 --seed-range 0-1
#   ./run_staghunt_iqn_tmux_four_conditions.sh --resource-density 0.15 --seeds 0,2,5
#   ./run_staghunt_iqn_tmux_four_conditions.sh --density 0.06 --seed 3
#   ./run_staghunt_iqn_tmux_four_conditions.sh --respawn-rate 0.02 --density 0.06
#
...
#
# Session names default from run_staghunt_iqn_tmux.sh (unique per group x punishment x density x seed).

set -e

SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
RUNNER="$SCRIPT_DIR/run_staghunt_iqn_tmux.sh"
...
echo "Launching ${_total} IQN sessions (${_nseeds} seed(s) x 2 conditions), RESOURCE_DENSITY=$RESOURCE_DENSITY RESPAWN_RATE=$RESPAWN_RATE"
