...
This module handles the execution of probe tests during training and saves
results to CSV files for analysis.
"""
    ...
    for agent_id, original_agent in enumerate(experiment_env.agents):
        ...
        with open(csv_path, 'w', newline='') as csvfile:
            ...
            for test_epoch in range(test_epochs):
                ...
                writer.writerow(_create_csv_row(epoch, agent_id, 'individual', agent_metrics, test_epoch))
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        for test_epoch in range(test_epochs):
            ...
            for agent_id, agent_metrics in epoch_metrics.items():
                writer.writerow(_create_csv_row(epoch, agent_id, 'group', agent_metrics, test_epoch))
...
def _save_agent_metrics_to_csv(csv_path, epoch, agent_id, test_type, agent_metrics, test_epoch=0):
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerow(_create_csv_row(epoch, agent_id, test_type, agent_metrics, test_epoch))
...
def _get_csv_fieldnames():
    ...
    """
    return [
        'epoch', 'agent_id', 'test_type', 'test_epoch', 'attacks_to_hares', 'attacks_to_stags',
