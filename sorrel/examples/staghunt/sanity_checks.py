...
and edge cases to ensure the staghunt implementation works correctly.
"""
...
def test_payoff_matrix_validation(results: SanityCheckResults, world: StagHuntWorld):
    ...
    try:
        ...
        if len(payoff_matrix) != 2 or len(payoff_matrix[0]) != 2:
            results.add_result("Payoff Matrix Size", False, f"Expected 2x2, got {len(payoff_matrix)}x{len(payoff_matrix[0])}")
            return
        ...
        if payoff_matrix == expected_values:
            results.add_result("Payoff Matrix Values", True, f"Correct values: {payoff_matrix}")
        else:
            results.add_result("Payoff Matrix Values", False, f"Expected {expected_values}, got {payoff_matrix}")
        ...
        if is_correct_asymmetric:
            results.add_result("Payoff Matrix Asymmetric", True, "Matrix correctly represents asymmetric stag hunt payoffs")
        else:
            results.add_result("Payoff Matrix Asymmetric", False, "Matrix should be asymmetric for stag hunt")
    ...
    try:
        ...
        for inventory, expected, description in test_cases:
            ...
            if result == expected:
                ...
                print(f"  ✗ {description}: {inventory} -> {result} (expected {expected})")
                all_passed = False
        
        results.add_result("Strategy Determination", all_passed, f"Tested {len(test_cases)} cases")
...
def test_resource_collection(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if not resource_found:
            results.add_result("Resource Collection", False, "No resources found to test collection")
            return
        ...
        if not adjacent_pos:
            results.add_result("Resource Collection", False, "No empty position found for resource collection test")
            return
        ...
        if inventory_updated and ready_updated:
            results.add_result("Resource Collection", True, f"Inventory: {agent.inventory}, Ready: {agent.ready}")
        else:
            results.add_result("Resource Collection", False, f"Inventory not updated: {inventory_updated}, Ready not updated: {ready_updated}")
...
def test_interaction_beam_mechanics(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        results.add_result("Beam Not Ready", not can_interact, f"Agent not ready: {not can_interact}")
        
        # Test 2: Beam should not fire when on cooldown
        agent.ready = True
        agent.beam_cooldown_timer = 2
        can_interact = agent.ready and agent.beam_cooldown_timer == 0
        results.add_result("Beam Cooldown", not can_interact, f"Agent on cooldown: {not can_interact}")
        
        # Test 3: Beam should fire when ready and not on cooldown
        agent.ready = True
        agent.beam_cooldown_timer = 0
        can_interact = agent.ready and agent.beam_cooldown_timer == 0
        results.add_result("Beam Ready", can_interact, f"Agent ready to interact: {can_interact}")
...
def test_observation_space_consistency(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if obs_size == model_input_size:
            results.add_result("Observation Space Consistency", True, f"Obs size: {obs_size}, Model size: {model_input_size}")
        else:
            results.add_result("Observation Space Consistency", False, f"Obs size: {obs_size}, Model size: {model_input_size}")
            
    except Exception as e:
        print(f"  Exception details: {str(e)}")
        import traceback
        traceback.print_exc()
        results.add_result("Observation Space Consistency", False, f"Exception: {str(e)}")
...
def test_action_space_validity(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        expected_actions = ["NOOP", "FORWARD", "BACKWARD", "STEP_LEFT", "STEP_RIGHT", "TURN_LEFT", "TURN_RIGHT"]
        ...
        if isinstance(action_spec.actions, list):
            ...
            results.add_result("Action Space Validity", False, f"Unexpected actions type: {type(action_spec.actions)}")
            return
        
        if actual_actions == expected_actions:
            results.add_result("Action Space Validity", True, f"Actions: {actual_actions}")
        else:
            results.add_result("Action Space Validity", False, f"Expected: {expected_actions}, Got: {actual_actions}")
        ...
        for i, action_name in enumerate(actual_actions):
            ...
            if mapped_action == action_name:
                ...
                results.add_result("Action Mapping", False, f"Action {i} maps to {mapped_action}, expected {action_name}")
                return
...
def test_reward_structure_validation(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if expected_taste_reward == 0.1:
            results.add_result("Taste Reward Value", True, f"Taste reward: {expected_taste_reward}")
        else:
            results.add_result("Taste Reward Value", False, f"Expected 0.1, got {expected_taste_reward}")
        
        # Test interaction reward
        expected_interaction_reward = agent.interaction_reward
        if expected_interaction_reward == 1.0:
            results.add_result("Interaction Reward Value", True, f"Interaction reward: {expected_interaction_reward}")
        else:
            results.add_result("Interaction Reward Value", False, f"Expected 1.0, got {expected_interaction_reward}")
        ...
        results.add_result("Payoff Matrix Values", all_payoffs_correct, f"Tested {len(expected_payoffs)} payoff combinations")
...
def test_world_bounds_and_movement(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        results.add_result("Valid Locations", valid_passed, f"Tested {len(valid_locations)} valid locations")
        
        # Test invalid locations
        invalid_passed = all(not world.valid_location(loc) for loc in invalid_locations)
        results.add_result("Invalid Locations", invalid_passed, f"Tested {len(invalid_locations)} invalid locations")
...
def test_episode_termination(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if not world.is_done:
            results.add_result("Episode Initial State", True, "Episode starts as not done")
        else:
        ...
        results.add_result("Termination Logic", has_termination, f"World has is_done attribute: {has_termination}")
...
def test_empty_inventory_interactions(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if not agent.ready:
            results.add_result("Empty Inventory Ready State", True, "Agent with empty inventory is not ready")
        else:
            results.add_result("Empty Inventory Ready State", False, "Agent with empty inventory is ready")
        ...
        if agent.ready:
            results.add_result("Single Resource Ready State", True, "Agent with single resource is ready")
        else:
            results.add_result("Single Resource Ready State", False, "Agent with single resource is not ready")
        
    except Exception as e:
        results.add_result("Empty Inventory Interactions", False, f"Exception: {str(e)}")
...
def test_beam_cooldown_enforcement(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if agent.beam_cooldown_timer == 2:
            results.add_result("Beam Cooldown Decrement", True, f"Cooldown decremented: {agent.beam_cooldown_timer}")
        else:
            results.add_result("Beam Cooldown Decrement", False, f"Expected 2, got {agent.beam_cooldown_timer}")
        ...
        if agent.beam_cooldown_timer == 0:
            results.add_result("Beam Cooldown Zero", True, f"Cooldown reached zero: {agent.beam_cooldown_timer}")
        else:
            results.add_result("Beam Cooldown Zero", False, f"Expected 0, got {agent.beam_cooldown_timer}")
...
def test_resource_collection_runtime(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if resource_collected:
            results.add_result("Resource Collection Runtime", True, f"Resource collected in {step+1} steps, inventory: {agent.inventory}")
        else:
            results.add_result("Resource Collection Runtime", False, "No resource collected after 10 steps")
            
    except Exception as e:
        results.add_result("Resource Collection Runtime", False, f"Exception: {str(e)}")


def test_observation_space_consistency_runtime(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        print(f"  Runtime observation size: {obs_size}, Model input size: {model_input_size}")
        
        if obs_size == model_input_size:
            results.add_result("Observation Space Consistency Runtime", True, f"Obs size: {obs_size}, Model size: {model_input_size}")
        else:
            results.add_result("Observation Space Consistency Runtime", False, f"Obs size: {obs_size}, Model size: {model_input_size}")
            
    except Exception as e:
        print(f"  Runtime observation exception: {str(e)}")
        results.add_result("Observation Space Consistency Runtime", False, f"Exception: {str(e)}")
...
def test_game_loop_execution(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if steps_executed > 0:
            results.add_result("Game Loop Execution", True, f"Executed {steps_executed} steps, active agents: {active_agents}")
        else:
        ...
        if reward_changed:
            results.add_result("Game Activity", True, f"Total reward changed from {initial_total_reward} to {world.total_reward}")
        else:
...
def test_agent_interactions_runtime(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if len(agents) < 2:
            results.add_result("Agent Interactions Runtime", False, "Need at least 2 agents for interaction test")
            return
        ...
        if interaction_occurred:
            results.add_result("Agent Interactions Runtime", True, f"Interaction occurred in {step+1} steps")
        else:
            results.add_result("Agent Interactions Runtime", False, "No interactions occurred after 10 steps")
...
def test_resource_respawn_mechanics(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if final_resources > 0:
            results.add_result("Resource Respawn Mechanics", True, f"Resources present: {final_resources}")
        else:
            results.add_result("Resource Respawn Mechanics", False, "No resources found after respawn period")
...
def test_agent_respawn_mechanics(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if final_active > 0:
            results.add_result("Agent Respawn Mechanics", True, f"Active agents: {final_active}")
        else:
            results.add_result("Agent Respawn Mechanics", False, "No active agents after respawn period")
...
def test_learning_behavior_runtime(results: SanityCheckResults, env: StagHuntEnv):
    ...
    try:
        ...
        if 0 <= action < agent.action_spec.n_actions:
            results.add_result("Learning Behavior Runtime", True, f"Agent selected valid action: {action}")
        else:
            results.add_result("Learning Behavior Runtime", False, f"Agent selected invalid action: {action}")
        ...
        if isinstance(reward, (int, float)):
            results.add_result("Action Execution Runtime", True, f"Action executed with reward: {reward}")
        else:
            results.add_result("Action Execution Runtime", False, f"Action execution returned invalid reward: {reward}")
        
        # Test that model memory works
        memory_state = agent.model.memory.current_state()
        if memory_state is not None:
            results.add_result("Model Memory Runtime", True, f"Memory state shape: {memory_state.shape}")
        else:
...
def run_all_sanity_checks():
    ...
    try:
        ...
        print(f"Created test environment: {world.height}x{world.width} with {len(env.agents)} agents")
        print()
        ...
        results.add_result("Environment Setup", False, f"Failed to create test environment: {str(e)}")
