...
class StagHuntPygame:
    ...
    def __init__(self, config=None):
        ...
        self.screen_height = (
            config["world"]["height"] * self.cell_size + 100
        )  # Extra space for UI
        ...
        for y in range(self.world.height):
            for x in range(self.world.width):
                ...
                if isinstance(entity, InteractionBeam):
                    ...
                    self.screen.blit(
                        beam_surface, (x * self.cell_size, y * self.cell_size)
                    )
    ...
    def draw_agent(self, x, y, orientation):
        ...
        # Draw agent body
        pygame.draw.circle(
            self.screen, self.colors["agent"], (center_x, center_y), self.cell_size // 3
        )
        ...
        pygame.draw.line(
            self.screen, self.colors["text"], (center_x, center_y), arrow_end, 3
        )
    ...
    def draw_ui(self):
        ...
        score_text = self.font_large.render(
            f"Score: {self.score:.1f}", True, self.colors["text"]
        )
        self.screen.blit(score_text, (10, ui_y + 10))

        # Turn count
        turn_text = self.font_medium.render(
            f"Turn: {self.turn_count}", True, self.colors["text"]
        )
        ...
        while spawned < num_resources:
            ...
            if isinstance(entity, Empty):
                ...
                if np.random.random() < 0.3:  # 30% chance for stag
                    resource_type = "stag"
                    resource = StagResource(
                        self.config["world"]["taste_reward"],
                        self.config["world"]["destroyable_health"],
                    )
                else:  # 70% chance for hare
                    resource_type = "hare"
                    resource = HareResource(
                        self.config["world"]["taste_reward"],
                        self.config["world"]["destroyable_health"],
                    )
