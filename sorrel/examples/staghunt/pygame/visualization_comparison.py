...
def compare_visualizations():
    ...
    print(
        f"ASCII map loaded: {hasattr(world, 'map_generator') and world.map_generator is not None}"
    )
    print()
    ...
    try:
        ...
        ascii_map_path = (
            Path(__file__).parent.parent / "docs" / "stag_hunt_ascii_map_clean.txt"
        )
        with open(ascii_map_path) as f:
