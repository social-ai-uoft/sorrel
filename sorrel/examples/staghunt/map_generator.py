...
This module provides functionality to parse ASCII map files and generate
world layouts based on the map specifications. The parser strictly follows
the map layout without adding extra entities.
"""
...
class MapBasedWorldGenerator:
    """Parses ASCII map files and provides exact layout data without adding extra entities.
    ...
    def _load_map(self) -> List[str]:
        ...
        for line in lines:
            ...
            if line and not line.startswith(('Legend:', 'W –', 'P –', '1 –', '2 –', 'a –', '(space)')):
                map_lines.append(line)
        
        if not map_lines:
            raise ValueError(f"Map file is empty or contains no valid map data: {self.map_file_path}")
    ...
    def _get_dimensions(self) -> Tuple[int, int]:
        ...
        for i, row in enumerate(self.raw_map):
            if len(row) != width:
                raise ValueError(f"Map row {i} has inconsistent width: {len(row)} vs {width}")
    ...
    def check_overlaps(self, map_data: MapLayoutData = None) -> dict:
        ...
        overlaps = {
            'spawn_vs_walls': self._find_overlaps(map_data.spawn_points, map_data.wall_locations),
            'spawn_vs_resources': self._find_overlaps(map_data.spawn_points, resource_coords),
            'spawn_vs_empty': self._find_overlaps(map_data.spawn_points, map_data.empty_locations),
            'walls_vs_resources': self._find_overlaps(map_data.wall_locations, resource_coords),
            'walls_vs_empty': self._find_overlaps(map_data.wall_locations, map_data.empty_locations),
            'resources_vs_empty': self._find_overlaps(resource_coords, map_data.empty_locations),
    ...
    def _find_overlaps(self, list1: List[Tuple[int, int]], list2: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
    ...
    def _find_duplicates(self, coord_list: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
        ...
        for overlap_type, overlap_list in overlaps.items():
            if overlap_list:
                if 'vs' in overlap_type:
                    ...
                    summary_parts.append(f"{type1} vs {type2}: {len(overlap_list)} overlaps at {overlap_list}")
                else:
                    summary_parts.append(f"Duplicate {overlap_type}: {len(overlap_list)} duplicates at {overlap_list}")
    ...
    def get_map_info(self) -> dict:
        ...
        return {
            ...
            'stag_resources': len([r for r in map_data.resource_locations if r[2] == 'stag']),
            'hare_resources': len([r for r in map_data.resource_locations if r[2] == 'hare']),
            'random_resources': len([r for r in map_data.resource_locations if r[2] == 'random']),
