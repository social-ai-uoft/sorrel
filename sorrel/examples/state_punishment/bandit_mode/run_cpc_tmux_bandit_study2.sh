#!/usr/bin/env bash
# Bandit analogue of study 2 (run_cpc_tmux_study2.sh): same CPC sweep, without punishment-level observability.
# Four tmux sessions: PPO-LSTM-CPC cpc_weight 0/1 x IQN+CPC cpc_weight 0/1 (weights 0.0 vs 0.1).
...
JOB="$SCRIPT_DIR/run_cpc_tmux_bandit_study2_job.sh"
...
# tmux new-session -d -s bandit_study_2_cpc_00_ppo
# tmux send-keys -t bandit_study_2_cpc_00_ppo "bash '$JOB' ppo_00" C-m

# tmux new-session -d -s bandit_study_2_cpc_01_ppo
# tmux send-keys -t bandit_study_2_cpc_01_ppo "bash '$JOB' ppo_01" C-m

created=()
for SEED in $(seq "$SEED_START" "$SEED_END"); do
  s1="bandit_study_2_cpc_01_iqn_k${K_ARMS}_s${SEED}"
  s0="bandit_study_2_cpc_00_iqn_k${K_ARMS}_s${SEED}"
