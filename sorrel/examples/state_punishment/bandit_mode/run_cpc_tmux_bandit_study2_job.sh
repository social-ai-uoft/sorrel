#!/usr/bin/env bash
# Bandit study 2 CPC job runner. Invoked by run_cpc_tmux_bandit_study2.sh: ppo_00|ppo_01|iqn_00|iqn_01
# Mirrors run_cpc_tmux_study2.sh knobs where bandit mode supports them (no grid/replacement/composite_views).
...
# Shared bandit study2 settings: K arms per trial; factored head Kx3 => grid-style move×vote mapping
COMMON=(
...
case "$1" in
  ...
  #     --run_folder_prefix np_00_cpc_ppo_lstm_cpc_bandit_study2_k${K_ARMS}_seed${SEED}
  #   ;;
  # ppo_01)
  #   exec python "${COMMON[@]}" \
  #     --model_type ppo_lstm_cpc \
  ...
  #     --ppo_use_factored_actions \
  #     --ppo_action_dims "${K_ARMS},3" \
  #     --run_folder_prefix np_01_cpc_ppo_lstm_cpc_bandit_study2_k${K_ARMS}_seed${SEED}
  #   ;;
  iqn_01)
    exec python "${COMMON[@]}" \
      --model_type iqn \
      ...
      --run_folder_prefix np_01_cpc_iqn_lstm_cpc_bandit_study2_k${K_ARMS}_seed${SEED}
    ;;
  iqn_00)
    exec python "${COMMON[@]}" \
      --model_type iqn \
      ...
      --iqn_use_factored_actions \
      --iqn_action_dims "${K_ARMS},3" \
      --run_folder_prefix np_00_cpc_iqn_lstm_cpc_bandit_study2_k${K_ARMS}_seed${SEED}
    ;;
  *)
    echo "Usage: $0 {iqn_00|iqn_01} [seed] [bandit_arms_per_trial]" >&2
    exit 1
    ;;
esac
