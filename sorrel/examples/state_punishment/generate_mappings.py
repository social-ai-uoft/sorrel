#!/usr/bin/env python3
"""Standalone script for generating entity appearance mappings with position
diversity."""
...
def _shuffle_no_fixed(entities: List[str]) -> Dict[str, str]:
    ...
    for attempt in range(max_attempts):
        ...
        if not any(
            entity == shuffled_appearances[i] for i, entity in enumerate(entities)
...
def is_position_diverse(
    new_mapping: Dict[str, str], position_history: List[Dict[str, str]]
) -> bool:
...
def generate_mapping_with_diversity(
    entities: List[str], constraint: str, position_history: List[Dict[str, str]]
) -> Dict[str, str]:
    ...
    print(
        f"Warning: Could not find position-diverse mapping after {max_attempts} attempts"
    )
...
def generate_diverse_mappings(
    ...
    if ensure_diversity:
        ...
        if num_mappings > max_possible:
            print(
                f"Warning: Requested {num_mappings} diverse mappings, but maximum possible is {max_possible}."
            )
    ...
    for i in range(num_mappings):
        if ensure_diversity:
            mapping = generate_mapping_with_diversity(
                entities, constraint, position_history
            )
        else:
            # For no_fixed constraint, ensure adjacent mappings don't share components
            if constraint == "no_fixed" and mappings:
                mapping = generate_no_fixed_with_adjacent_diversity(
                    entities, mappings[-1]
                )
            else:
                mapping = generate_random_mapping(entities, constraint)
...
def generate_no_fixed_with_adjacent_diversity(
    entities: List[str], previous_mapping: Dict[str, str]
) -> Dict[str, str]:
    """Generate a no_fixed mapping that doesn't share any components with the previous
    mapping."""
    max_attempts = 1000
    ...
    for attempt in range(max_attempts):
        ...
        if not any(
            ...
            for entity in entities:
                if (
                    shuffled_appearances[entities.index(entity)]
                    == previous_mapping[entity]
                ):
...
def calculate_diversity_stats(mappings: List[Dict[str, str]]) -> Dict[str, float]:
    ...
    diversity_ratio = (
        1.0 - (conflicts / total_positions) if total_positions > 0 else 1.0
    )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="Output file path for mappings"
    )
    parser.add_argument(
