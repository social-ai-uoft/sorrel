...
def setup_probe_test_environment(config, args, use_important_rule=None):
    ...
    probe_multi_agent_env, probe_shared_state_system, probe_shared_social_harm = setup_environments(
        probe_config, args.simple_foraging, args.fixed_punishment, args.random_policy, 
...
def copy_and_freeze_model_weights(source_model, target_model):
    ...
    target_model.qnetwork_local.load_state_dict(source_model.qnetwork_local.state_dict())
    # Copy target network weights  
    target_model.qnetwork_target.load_state_dict(source_model.qnetwork_target.state_dict())
...
def run_probe_test(training_env, probe_env, training_epoch: int, probe_epochs: int = 10):
    ...
    for train_env, probe_env in zip(training_env.individual_envs, probe_env.individual_envs):
