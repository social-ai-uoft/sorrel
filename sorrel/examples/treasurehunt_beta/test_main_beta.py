...
class TestLogger(Logger):
    ...
    def record_evaluation(self, training_epoch, eval_epoch, total_reward, individual_scores, encounters):
        ...
        print(f"EVALUATION at epoch {training_epoch}: Total reward = {total_reward:.2f}, "
...
def run_test_training_with_evaluation():
    ...
    log_dir = Path(__file__).parent / f'test_runs/{config["experiment"]["run_name"]}_{timestamp}'
    ...
    print(f"Evaluation frequency: every {config['experiment']['eval_frequency']} epochs")
    ...
    if logger.evaluation_results:
        ...
        for result in logger.evaluation_results:
            print(f"Epoch {result['training_epoch']}: "
