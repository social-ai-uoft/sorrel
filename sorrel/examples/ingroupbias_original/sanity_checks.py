...
cases to ensure the ingroup bias implementation works correctly.
"""
...
def test_entity_creation():
    ...
    results.add_result(
        ...
        red_resource.name == "red"
        and red_resource.color == "red"
        and red_resource.passable == True,
        f"Name: {red_resource.name}, Color: {red_resource.color}, Passable: {red_resource.passable}",
    )

    results.add_result(
        "GreenResource creation",
        green_resource.name == "green"
        and green_resource.color == "green"
        and green_resource.passable == True,
        f"Name: {green_resource.name}, Color: {green_resource.color}, Passable: {green_resource.passable}",
    )

    results.add_result(
        "BlueResource creation",
        blue_resource.name == "blue"
        and blue_resource.color == "blue"
        and blue_resource.passable == True,
...
def test_world_initialization():
    ...
    try:
        ...
        results.add_result(
            "Layer indices",
            world.terrain_layer == 0
            and world.dynamic_layer == 1
            and world.beam_layer == 2,
        ...
        results.add_result(
            ...
            world.num_agents == 2
            and world.resource_density == 0.3
            and world.beam_length == 2,
        ...
        expected_walls = (
            2 * (world.height + world.width) - 4
        )  # Perimeter minus corners counted twice
        results.add_result(
            "Wall placement",
...
def test_agent_creation():
    ...
    try:
        ...
        # Test agent count
        results.add_result(
            "Agent count", len(env.agents) == 2, f"Number of agents: {len(env.agents)}"
        )
        ...
        results.add_result(
            ...
            agent.orientation == 0
            and agent.ready == False
            and agent.inventory == {"red": 0, "green": 0, "blue": 0},
        ...
        actual_actions = [
            agent.action_spec.get_readable_action(i)
            for i in range(agent.action_spec.n_actions)
        ]
        results.add_result(
            "Action names",
...
def test_resource_collection():
    ...
    try:
        ...
        results.add_result(
            "Resource collection",
            new_pos == resource_pos
            and agent.inventory["red"] == 1
            and agent.ready == True,
        ...
        results.add_result(
            "Resource removal",
            isinstance(
                entity_at_pos, IngroupBiasAgent
            ),  # Agent moved to resource position
...
def test_interaction_beam():
    ...
    try:
        ...
        # Debug information
        print(
            f"Debug: Agent1 at {agent1.location}, orientation {agent1.orientation}, ready {agent1.ready}"
        )
        ...
        interaction_occurred = (
            ...
            and agent2.inventory == {"red": 0, "green": 0, "blue": 0}
        )
...
def test_frozen_and_respawn_states():
    ...
    try:
        ...
        print(
            f"Debug: After interaction - Agent1 at {agent1.location}, frozen: {world.is_agent_frozen(agent1)}"
        )
        print(
            f"Debug: After interaction - Agent2 at {agent2.location}, frozen: {world.is_agent_frozen(agent2)}"
        )
        ...
        results.add_result(
            ...
            not world.is_agent_frozen(agent1)
            and world.is_agent_respawning(agent1)
            and not world.is_agent_frozen(agent2)
            and world.is_agent_respawning(agent2),
...
def test_reward_calculation():
    ...
    try:
        ...
        # Debug information
        print(
            f"Debug: Before interaction - Agent1 at {agent1.location}, ready: {agent1.ready}, inventory: {agent1.inventory}"
        )
        print(
            f"Debug: Before interaction - Agent2 at {agent2.location}, ready: {agent2.ready}, inventory: {agent2.inventory}"
        )
        ...
        interaction_occurred = (
            ...
            or (reward > 0)
        )  # Interaction occurred if reward > 0

        print(
            f"Debug: After interaction - Agent1 at {agent1.location}, frozen: {world.is_agent_frozen(agent1)}, inventory: {agent1.inventory}"
        )
        print(
            f"Debug: After interaction - Agent2 at {agent2.location}, frozen: {world.is_agent_frozen(agent2)}, inventory: {agent2.inventory}"
        )
        ...
        reward = agent1.act(world, 8)  # interact
        results.add_result(
            "Zero reward with empty inventories", reward == 0.0, f"Reward: {reward}"
        )
...
def test_observation_system():
    ...
    try:
        ...
        expected_size = (
            7 * 7 * 1 * 9 + 4
        )  # 7x7 grid, 1 layer, 9 entity types + 4 features
        results.add_result(
            "Observation size",
    ...
    for test_func in test_functions:
        ...
        try:
            ...
            all_results.add_result(
                test_func.__name__, False, f"Test function failed: {str(e)}"
            )
