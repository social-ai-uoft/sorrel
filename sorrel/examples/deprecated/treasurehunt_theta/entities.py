"""The entities for treasurehunt_theta, a modified treasurehunt with three resource types."""
...
class Sand(Entity[TreasurehuntThetaWorld]):
    """An entity that represents a block of sand in the treasurehunt_theta environment."""
...
class HighValueResource(Entity[TreasurehuntThetaWorld]):
    """An entity that represents a high-value resource (value 15) in the treasurehunt_theta environment."""

    def __init__(self):
        super().__init__()
        self.passable = True  # Agents can move onto HighValueResource
        self.value = 15
        self.sprite = Path(__file__).parent / "./assets/coin.png"  # Using coin sprite for high value


class MediumValueResource(Entity[TreasurehuntThetaWorld]):
    """An entity that represents a medium-value resource (value 5) in the treasurehunt_theta environment."""

    def __init__(self):
        super().__init__()
        self.passable = True  # Agents can move onto MediumValueResource
        self.value = 5
        self.sprite = Path(__file__).parent / "./assets/food.png"  # Using food sprite for medium value


class LowValueResource(Entity[TreasurehuntThetaWorld]):
    """An entity that represents a low-value resource (value -5) in the treasurehunt_theta environment."""

    def __init__(self):
        super().__init__()
        self.passable = True  # Agents can move onto LowValueResource
        self.value = -5
        self.sprite = Path(__file__).parent / "./assets/bone.png"  # Using bone sprite for low value


class EmptyEntity(Entity[TreasurehuntThetaWorld]):
    """An entity that represents an empty space in the treasurehunt_theta environment."""
    ...
    def transition(self, world: TreasurehuntThetaWorld):
        ...
        probabilities dictated in the environment. With equal probability for each resource type."""
