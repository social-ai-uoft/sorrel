...
def run_training_with_evaluation():
    ...
    parser = argparse.ArgumentParser(description='Run treasurehunt_theta experiment with evaluation')
    parser.add_argument('--experiment_name', type=str, default='treasurehunt_theta_eval',
                        help='Name of the experiment (default: treasurehunt_theta_eval)')
    parser.add_argument('--log_dir', type=str, default='runs',
                        help='Directory for logging (default: runs)')
    parser.add_argument('--respawn_prob', type=float, default=0.0,
    ...
    print(f"Epsilon decay: {config['model']['epsilon_decay']}, Epsilon min: {config['model']['epsilon_min']}")
    ...
    for epoch in range(config["experiment"]["epochs"]):
        ...
        if (epoch + 1) % eval_frequency == 0:
            ...
            for i, agent in enumerate(experiment.agents):
                checkpoint_path = model_checkpoint_dir / f"agent_{i}_epoch_{epoch + 1}.pkl"
