...
def parse_args():
    ...
    parser.add_argument(
        "--model_type", help="type of model to use", default="iRainbowModel"
    )
...
def create_models(cfg):
    ...
    for model_name in vars(cfg.model):
        ...
        for _ in range(vars(vars(cfg.model)[model_name])["num"]):
            model = MODEL_TYPE(
                **vars(vars(vars(cfg.model)[model_name])["parameters"]),
                device="cpu",
                seed=1,
            )
...
def create_agents(cfg, models):
    ...
    for agent_type in vars(cfg.agent):
        ...
        for ixs in range(vars(vars(cfg.agent)[agent_type])["num"]):
            ...
            agents.append(AGENT_TYPE(agent_model, cfg, ixs))
    ...
    for entity_type in vars(cfg.entity):
        ...
        if "start_num" in vars(vars(cfg.entity)[entity_type]):
            for _ in range(vars(vars(cfg.entity)[entity_type])["start_num"]):
                entities.append(
                    ENTITY_TYPE(
                        color_map(cfg.env.channels)[entity_type],
                        cfg,
...
def save_config_backup(config_file_path, backup_dir):
    """Saves a backup of the YAML configuration file with a timestamp and 'exp_name' as
    part of the name.
...
def safe_get_size(obj, seen=None):
    ...
    try:
        if isinstance(obj, dict):
            size += sum(
                safe_get_size(k, seen) + safe_get_size(v, seen) for k, v in obj.items()
            )
...
def print_top_largest_tensors(n=10):
    ...
    tensor_sizes = [
        (type(t), t.size(), t.element_size() * t.nelement() / 1024 / 1024)
    ...
    for i, (typ, shape, size_mb) in enumerate(tensor_sizes[:n]):
        print(
            f"{i+1:>2}. {typ.__name__:<15} shape={tuple(shape)} size={size_mb:.2f} MB"
        )
