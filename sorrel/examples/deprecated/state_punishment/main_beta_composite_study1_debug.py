...
def run(cfg, **kwargs):
    ...
    agents: list[Agent] = create_agents(cfg, models) # check
    for a in agents:
        print(a.appearance)
    ...
    if cfg.log:
        ...
        writer = SummaryWriter(
            log_dir=f'{root}/examples/state_punishment/runs_collective/{cfg.exp_name}_{datetime.now().strftime("%Y%m%d-%H%m%s")}/'
        )
    ...
    for epoch in range(cfg.experiment.epochs):
        ...
        # set the punishment level manually if needed (when set level manually, the action space should not include voting)
        if cfg.state_sys.manual_punishment_level is not None:
            state_entity.prob = cfg.state_sys.manual_punishment_level * state_entity.change_per_vote
            state_entity.init_prob = cfg.state_sys.manual_punishment_level * state_entity.change_per_vote
            state_entity.level = cfg.state_sys.manual_punishment_level
        ...
        while not done:
            ...
            for ixs, agent in enumerate(agents):
                # # debug 
                # print(f'agent{agent.ixs}, preaction_state_punishment_level{state_entity.level, state_entity.prob}')
                ...
                #     reward = 0
                #     # print(epoch%2, state_entity.prob, state_entity.level)
                #     if state_entity.level == action:
                #         reward = 1
                    
                            
                
                # # debug
                # print(f'agent{agent.ixs}, action: {action}, after_state_punishment_level{state_entity.level, state_entity.prob}')
