...
class Agent:
    def __init__(self, model, cfg, ixs):
        ...
        if isinstance(cfg.agent.agent.appearance, str):
            ...
            self.appearance = cfg.agent.agent.appearance[
                ixs
            ]  # agents are blue # cfg.agent.agent.appearance[ixs]?
        ...
        self.is_punished = 0.0
        self.to_be_punished = {"Gem": 0, "Bone": 0, "Coin": 0}
        self.transgression_record = {
            resource: [] for resource in cfg.state_sys.resources
        }
    ...
    def init_replay(self, env: Gridworld, state_mode: str) -> None:
        ...
        if state_mode == "simple":
            state = np.zeros_like(
                np.concatenate([self.pov(env), np.zeros(self.extra_percept_size)])
            )
        else:
            grid_state = self.generate_composite_state([env for _ in range(6)])
            state = np.zeros_like(
                np.concatenate([grid_state, np.zeros(self.extra_percept_size)])
            )
    ...
    def movement(self, action: int, state_sys, mode="simple") -> tuple:
        ...
        if mode == "simple":
            ...
            if action == 1:  # DOWN
                self.sprite = (
                    f"{self.cfg.root}/examples/state_punishment/assets/hero.png"
                )
                new_location = (
                    self.location[0] + 1,
                    self.location[1],
            ...
            if action == 2:  # LEFT
                self.sprite = (
                    f"{self.cfg.root}/examples/state_punishment/assets/hero-left.png"
                )
                new_location = (
                    self.location[0],
                    self.location[1] - 1,
            ...
            if action == 3:  # RIGHT
                self.sprite = (
                    f"{self.cfg.root}/examples/state_punishment/assets/hero-right.png"
                )
                new_location = (
                    self.location[0],
                    self.location[1] + 1,
    ...
    def transition(
        ...
        if str(target_object) in state_sys.potential_taboo:
            ...
            if mode == "certain":
                ...
                if state_sys.resource_punishment_is_ambiguous:
                    punishment_prob = state_sys.punishment_schedule_func(
                        str(target_object)
                    )
                    # val_of_state_punishment = state_sys.magnitude * (random.random() < punishment_prob)
                    # deterministic punishment
                    val_of_state_punishment = state_sys.magnitude * punishment_prob
                    ...
                    if state_sys.only_punish_taboo:
                        if str(target_object) in state_sys.taboo:
                            ...
                            # val_of_state_punishment = state_sys.magnitude * (random.random() < punishment_prob)
                            # deterministic punishment
                            val_of_state_punishment = (
                                state_sys.magnitude * punishment_prob
                            )
                    ...
                    # use a fixed list of probabilities to determine punishment
                        ...
                        val_of_state_punishment = (
                            state_sys.magnitude
                            * state_sys.prob_list[str(target_object)]
                            * state_sys.prob
                        )
                ...
                self.punishment_record[str(target_object)].append(
                    1 * (val_of_state_punishment > 0)
                )

            cache_harm = [
                (
                    env.cache["harm"][k] - target_object.social_harm
                    if k != self.ixs
                    else env.cache["harm"][k]
                )
        ...
        next_state = np.concatenate([next_state, np.array([random.random() * 255])])
        # append whether anyone has committed a crime or not by showing the cached harm in the society
        next_state = np.concatenate(
            [next_state, np.array([env.cache["harm"][self.ixs] * 255])]
        )
    ...
    def reset_record(self) -> None:
        self.transgression_record = {
            resource: [] for resource in self.cfg.state_sys.resources
        }
...
def color_map(channels: int) -> dict:
    ...
    if channels > 5:
        ...
        colors = {
            "EmptyObject": [255.0, 255.0, 255.0],
            "Agent": [0.0, 0.0, 255.0],
            "Wall": [153.0, 51.0, 102.0],
            "Gem": [0.0, 255.0, 0.0],
            "Coin": [255.0, 255.0, 0.0],
            "Food": [255.0, 0.0, 0.0],
            "Bone": [0.0, 0.0, 0.0],
        }
    return colors
