...
class RollingAverageLogger(Logger):
    ...
    def __init__(
        self, max_epochs: int, window_size: int = 100, print_freq: int = 100, *args
    ):
    ...
    def record_turn(self, epoch, loss, reward, epsilon=0, **kwargs):
        ...
        filtered_kwargs = {
            k: v for k, v in kwargs.items() if k in self.additional_values
        }
        ...
        if epoch % self.print_freq == 0 and epoch > 0:
            ...
            print(
                f"Epoch {epoch}: Avg Reward (last {len(recent_rewards)}): {avg_reward:.2f}, Avg Loss: {avg_loss:.2f}, Epsilon: {epsilon:.4f}",
                flush=True,
            )
