...
class EpisodeBuffer:
    ...
    * ``sample_cpc`` returns a batch of transitions augmented with a future goal.
    ...
    removes old transitions once the total number of stored transitions exceeds
    ``capacity``.
    ...
    Attributes:
        capacity (int): maximum number of transitions to store.  When the
            buffer exceeds this size, the oldest transitions are dropped.
    ...
    def _ensure_capacity(self) -> None:
        """Ensure the buffer does not exceed its total transition capacity.

        If the number of stored transitions exceeds ``self.capacity``,
        transitions are removed from the oldest episodes until the total
        count is below or equal to the capacity.  Entire episodes may be
        removed if they become empty.
        """
        while self.num_transitions > self.capacity and self.episodes:
            # Remove transitions from the oldest episode
            ep = self.episodes[0]
            if ep['states']:
                # Pop the first transition from the episode
                ep['states'].pop(0)
                ep['actions'].pop(0)
                ep['rewards'].pop(0)
                done_flag = ep['dones'].pop(0)
                self.num_transitions -= 1
                # Update virtual index if we're removing from the front
                if self._idx > 0:
                    self._idx -= 1
                # If the episode ended at this removed step, clear remaining
                # transitions from the old episode because they belong to the
                # next logical episode.  This prevents mixing episodes after
                # trimming.
                if done_flag:
                    # drop the rest of the episode since the boundary has moved
                    ep['states'].clear()
                    ep['actions'].clear()
                    ep['rewards'].clear()
                    ep['dones'].clear()
                ...
                # If the episode has no transitions left, drop it entirely
                self.episodes.pop(0)
    ...
    def add(self, obs: np.ndarray, action: int, reward: float, done: bool) -> None:
        ...
        self.num_transitions += 1
        self._idx = (self._idx + 1) % self.capacity  # Update virtual index (Buffer compatibility)
        # Check capacity and drop old transitions if necessary
    ...
    def add_empty(self) -> None:
        ...
        """
        self._idx = (self._idx + self.n_frames - 1) % self.capacity
