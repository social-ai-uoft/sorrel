...
class Node:
  """
  Nodes are a class that can contain an indefinite number of 
  entities and agents. They connect to other nodes through
  being _adjacent_ to these nodes; entities at other nodes
  can also be _visible_ in a particular node depending on a
  node's visibility.
  ...
  def __str__(self):
    return pprint.pformat({
      ...
      "adjacent_nodes": [
        node.name for node in self.adjacent
      ]
    }, compact=True)
...
class NodeWorld:
  """World that represents a graph of nodes. Each node can
  contain agents and entities.
