...
class VisualEncoder(nn.Module):
    ...
    def __init__(self, input_channels: int = 1, use_variant1: bool = True):
        ...
        self.conv2 = nn.Conv2d(
            16 if use_variant1 else 6, 32, kernel_size=4, stride=1, padding=0
        )
...
class ActorCriticDeepMind(nn.Module):
    ...
    def evaluate(
        ...
        """
        action_probs, state_value, new_hidden, cpc_features = self.forward(
            state, hidden
        )
        dist = Categorical(action_probs)
...
class A2C_DeepMind(PyTorchModel):
    ...
    def __init__(
        ...
        self.optimizer = torch.optim.RMSprop(
            self.policy.parameters(), lr=lr, eps=1e-5, momentum=0.0, alpha=0.99
        )
    ...
    def train_step(self):
        ...
        states = torch.tensor(
            self.memory.states[: self.memory.size],
            dtype=torch.float32,
            device=self.device,
        )
        actions = torch.tensor(
            self.memory.actions[: self.memory.size],
            dtype=torch.long,
            device=self.device,
        )

        # Evaluate current policy
        log_probs, state_values, entropy, _, cpc_features = self.policy.evaluate(
            states, actions
        )
